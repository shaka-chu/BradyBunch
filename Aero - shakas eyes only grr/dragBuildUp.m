...
%     FF_t = FF_w;
    ...
    if mode == 3
        ...
        % Rudder Cd for 5 degree deflection
        Cd_rudder = 0.02;
        
        % Re-dimensionalise for wing
        Cd_rudder = Cd_rudder*S_t/S_ref;
        
        % Add to parasitic drag
        Cdmin = Cdmin + Cd_rudder;
        
    end
    
%     thingo = figure;
%     plot(Rwf(:,1),Rwf(:,2));
%     xlabel('Reynolds Number, Re','FontSize',18);
%     ylabel('Wing-Body Inteference Factor, R_{wb}','FontSize',18)
%     set(gca, 'XScale', 'log')
%     set(gca, 'XLimSpec', 'Tight');
%     set(gcf, 'Color', [1 1 1]);
%     set(gca, 'Color', [1 1 1]);
%     grid on
%     print(thingo,'Rwf','-depsc');
